...
func xmlToFileLogWriter(filename string, props []xmlProperty, enabled bool) (*FileLogWriter, bool) {
	...
	for _, prop := range props {
		...
		case "maxbuckup":
			maxbuckup = strToNumSuffix(strings.Trim(prop.Value, " \r\n"), 1)
		default:
	...
	if len(file) == 0 {
		...
		//文件名
		file = filepath.Base(osPath)

		dir = dir + path + "log"
		os.Mkdir(dir, os.ModePerm) //在当前目录下生成md目录
		// dir = dir + path + "golang"
		// os.Mkdir(dir, os.ModePerm) //在当前目录下生成md目录

		fmt.Fprintf(os.Stderr, "%v %v %v\n", osPath, dir, file)
		file = dir + path + file
		file = file + "_" + strconv.Itoa(os.Getpid())
	...
	flw.SetRotateMaxBackup(maxbuckup)
	return flw, true
}
...
func xmlToXMLLogWriter(filename string, props []xmlProperty, enabled bool) (*FileLogWriter, bool) {
	...
	xlw.SetRotateMaxBackup(maxbuckup)
	return xlw, true
}
